...
from utilities.loss_func import loss_func

from torch.utils.data import DataLoader, random_split
from utilities.vggloss import VGGPerceptualLoss
...
def train_net(net,
  ...
  dataset = BasicDataset(dir_img, fold=fold, patch_size=patchsz,
                         max_trdata=trimages)
  ...
  for epoch in range(epochs):
    ...
    with tqdm(total=n_train, desc=f'Epoch {epoch + 1}/{epochs}',
      ...
      for batch in train_loader:
        ...
        mapping_func, _ = net.global_net(histogram)
        mapping_func = torch.reshape(mapping_func, (imgs.size(0), 9, 3))
        corrected_global = imgs.detach()
        for i in range(mapping_func.size(0)):
          temp = torch.mm(net.kernel(torch.reshape(torch.squeeze(
            imgs[i, :, :, :]), (-1, 3))), torch.squeeze(mapping_func[i, :, :]))
          corrected_global[i, :, :, :] = torch.reshape(temp, (
            imgs.size(1), imgs.size(2), imgs.size(3)))


        loss = loss_func.compute(corrected_global, awb_gt)
        optimizer.zero_grad()
        loss.backward()
        optimizer.step()

        imgs_pred, imgs_pred_global = net(imgs, histogram)
        loss = loss_func.compute(imgs_pred, awb_gt,
                                 global_output=imgs_pred_global,
    ...
    if (epoch + 1) % validationFrequency == 0:
      ...
      logging.info('Validation error: {}'.format(val_score))
      if use_tb:
        writer.add_scalar('learning_rate', optimizer.param_groups[0]['lr'],
                          global_step)
        writer.add_scalar('Loss/test', val_score, global_step)
        writer.add_images('images', imgs, global_step)
        writer.add_images('result-awb (global)', imgs_pred_global, global_step)
        writer.add_images('result-awb', imgs_pred, global_step)
    ...
    if save_cp and (epoch + 1) % chkpointperiod == 0:
      ...
      torch.save(net.state_dict(),
                 dir_checkpoint + f'AWB_epoch{epoch + 1}.pth')
...
def vald_net(net, loader, device):
  ...
  with tqdm(total=n_val, desc='Validation round', unit='batch',
    ...
    for batch in loader:
      ...
      with torch.no_grad():
        imgs_pred, _ = net(imgs, histogram)
        loss = loss_func.compute(imgs_pred, awb_gt)
...
def get_args():
  parser = argparse.ArgumentParser(description='Train AWB network.')
  ...
  parser.add_argument('-v', '--validation', dest='val', type=float,
                      ...
                      help='Percent of the data that is used as validation'
                           ' (0-100)')
  parser.add_argument('-vf', '--validation-frequency', dest='val_frq', type=int,
                      default=1,
                      help='Validation frequency.')
  parser.add_argument('-d', '--fold', dest='fold', type=int, default=0,
                      help='Testing fold to be excluded. '
                           'Use --fold 0 to use all Set1 training data')
  parser.add_argument('-s', '--patch-size', dest='patchsz', type=int,
                      default=256,
                      help='Size of training patch')
  parser.add_argument('-t', '--num_training_images', dest='trimages', type=int,
                      default=60000,
                      help='Number of training images. Use '
                           '--num_training_images 0 to use all training images')
  ...
  parser.add_argument('-trd', '--training_dir', dest='trdir',
                      default='../data/',
...
if __name__ == '__main__':
  ...
  logging.info('Training of Auto White-Balance Network')
  args = get_args()
  ...
  #BasicDataset.compute_hist(args.trdir)
